...
### Ввод паролів виключно в терміналі

За замовчуванням пароль від ключа система буде запитувати в окремому віконці, щоб це вимкнути треба у файл ```~/.gnupg/gpg-agent.conf``` додати рядок ```pinentry-program /usr/bin/pinentry-tty```. Pynentry - це діалогові вікна і крім ttf-версії там є ще кілька графічних. Потім треба перезапустити gpg-agent:

```bash
systemd --user reload gpg-agent
```

Паролі у терміналі не відображаються.
...
## Перевіряємо основну зв'язку ключів

```bash
gpg --homedir /%флешка%/.gnupg -k

/%флешка%/.gnupg/pubring.kbx
------------------------
sec   4096R/0xD93D03C13478D580 2016-11-30 [expires: 2018-11-30] usage: C
      Key fingerprint = F8C8 1342 2A7F 7A3A 9027  E158 D93D 03C1 3478 D580
uid                            Alice
uid                            alice@example.org
ssb   4096R/0x1ED73636975EC6DE 2016-11-30 [expires: 2018-11-30] usage: S
      Key fingerprint = 292D 3E78 6B2E DBEA 1D10  02C8 1ED7 3636 975E C6DE
ssb   4096R/0x76737ABEB92745D7 2016-11-30 [expires: 2018-11-30] usage: E
      Key fingerprint = 0C33 42E5 670A B099 8ED7  3E87 7673 7ABE B927 45D7
ssb   4096R/0xE379FB0D81B6925D 2016-11-30 [expires: 2018-11-30] usage: A
      Key fingerprint = 7357 2158 947D BAFF A89F  4911 E379 FB0D 81B6 925D
```

Бачимо наступні значення:
- **sec** - секретний ключ;
- **ssb** - секретний підключ;
- **pub** - публічний ключ;
- **sub** - публічний підключ;
- **uid** - ідентифікатор користувача (User-ID);
- **ssb** - підключ;

- **4096R** - довжина ключа;

- **0xD93D03C13478D580** - ID ключа у форматі "0хlong";

- **2016-11-30** - дата коли був створений ключ;
- **2018-11-30** - дата коли збіжить термін придатності ключа (термін можна буде продовжити);

usage:
- **C - Certification** - сертифікація;
- **S - Signing** - підписання;
- **E - Encryption** - шифрування;
- **A - Authentication** - аутентифікація;
...
## Публікація ключів GnuPG

Необхідно опублікувати ключі на публічному сервері щоб їх могли перевірити ваші співрозмовники. Після публікації ключі неможливо буде видалити з серверу, їх можна тільки відкликати, але на сервері вони залишаться. Тому поспішати з цим кроком не слід.

```bash
$ gpg --send-keys 0xD93D03C13478D580
```
