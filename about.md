---
layout: page
title: Контакти
permalink: /about/
published: true
---

think4web - віртуальна тінь, що не існує за межами інтернет-мережі. Цей блог є лише спробою систематизації окремих нотаток, записів, перекладів на тему безпеки, конфіденційноті та вільного програмного забезпечення зроблених, у першу чергу, для себе. Тому пости можуть доповнюватися, змінюватися, переписуватись повністю.

Зв'язатися:
- [matrix](https://matrix.to/#/@jwtzbtlceh:here.in.ua)
- [pleroma](https://social.net.ua/think4web)
- [mail](mailto:efxice9c@anonaddy.me)

<details>
  <summary>PGP key</summary>
<pre><code>
-----BEGIN PGP PUBLIC KEY BLOCK-----


=hrIV
-----END PGP PUBLIC KEY BLOCK-----  
```
</code></pre>
</details>



